...
function dibujar()
	{
		...
		$campos = "id, Nombre, Carpeta, Anio,
				   ...
				   Archivo,Ruta,timg1_4";
				
		
		if ($condicionFinal == "") {
			$consultaPaginacion = "SELECT $campos  FROM adminarchivos order by id ASC"; //where cg.Empleado_Activo ='1'  
			
	

		}elseif ($condicionFinal != "")   {
			$consultaPaginacion = "SELECT $campos  FROM adminarchivos   $condicionFinal  order by id ASC"; //where cg.Empleado_Activo ='1'  
		...
		if (($Buscar) and ($condicionFinal == "")) {
		   //echo "entro en la opci?n buscar ";
			$consulta2 = "SELECT $campos  FROM adminarchivos order by id ASC 
			...
			$consulta2 = "SELECT $campos  FROM adminarchivos  $condicionFinal  order by id ASC 
			LIMIT $RegistrosAEmpezar, $RegistrosAMostrar"; //where cg.Empleado_Activo ='1'  

		}else { 
			//echo "entro en la opción else solas "
			$consulta2 = "SELECT $campos  FROM adminarchivos    order by id ASC LIMIT $RegistrosAEmpezar, $RegistrosAMostrar"; 
			//where cg.Empleado_Activo ='1'  
...
<div align="center"><a title="Agregar nuevos archivos al sitio" href="AdminArchivos.php?Accion=<?PHP echo $Accion_Agregar_Registros;?>"><img src="img/edit_add.png" border="0"></a>
...
<table width="59%"  align="center">
           <thead>
               ...
               <th width="5%"><strong>P&uacute;blico</strong></th>
                <th width="5%"><strong>Thumbnail</strong></th>
           ...
           <tbody>
			...
			?>
               ...
               <td><div align="center"><a class="imgsys" href="<?PHP echo $href; ?>"><img src="<?php echo $img?>"  alt="firefox" class="float-left" /></a></div></td>
			  
               <td><div align="center"><a href="<?PHP echo $ruta;?>">imagen</a></div></td>
			   <td align="center"><a href="AdminArchivos.php?id=<?PHP echo $row->id?>&Accion=<?PHP echo $AccionMod;?>"><img src="img/curso.gif" width="30" height="30" border="0" /></a></td>
               
                <td align="center"><span class = "dele"><a onClick="EliminarDato(<?php echo $row->id ?>); return false";  href="eliminarArchivosUnload.php?id=<?php echo $row->id;?>" title="Eliminar" alt="Eliminar"><img src="img/close.png" width="30" height="30" border="0" /></a></span></td>
...
<form id="formnoticias"   name="formnoticias" method="post" action="UnloadArchivosAdmin.php" enctype="multipart/form-data">
