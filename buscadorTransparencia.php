...
function dibujar()
	{
		...
		$campos = "id, Nombre, Enlace, Anio,
				   FechaSubida,Usuario,Publicar,
				   Archivo,Ruta";
				
		
		if ($condicionFinal == "") {
			$consultaPaginacion = "SELECT $campos  FROM trasnparencia order by id ASC"; //where cg.Empleado_Activo ='1'  
			
	

		}elseif ($condicionFinal != "")   {
			$consultaPaginacion = "SELECT $campos  FROM trasnparencia   $condicionFinal  order by id ASC"; //where cg.Empleado_Activo ='1'  
		...
		if (($Buscar) and ($condicionFinal == "")) {
		   //echo "entro en la opci?n buscar ";
			$consulta2 = "SELECT $campos  FROM trasnparencia order by id ASC 
			...
			$consulta2 = "SELECT $campos  FROM trasnparencia  $condicionFinal  order by id ASC 
			LIMIT $RegistrosAEmpezar, $RegistrosAMostrar"; //where cg.Empleado_Activo ='1'  

		}else { 
			//echo "entro en la opción else solas "
			$consulta2 = "SELECT $campos  FROM trasnparencia    order by id ASC LIMIT $RegistrosAEmpezar, $RegistrosAMostrar"; 
			//where cg.Empleado_Activo ='1'  
...
<div align="center"><a title="Agregar nuevos archivos al sitio de transparencia" href="transparenciaWeb.php?Accion=<?PHP echo $Accion_Agregar_Registros;?>"><img src="img/edit_add.png" border="0"></a>
...
<table width="59%"  align="center">
           <thead>
               ...
               <th width="5%"><strong>Publicar</strong></th>
			...
			?>
			   <tr id="fila-<?php echo $row->id; ?>" class="<?PHP echo $class;?>">       
                ...
                <td><div align="center"><?PHP echo utf8_encode($row->Nombre);?></div></td>
   		   	   <td><div align="center"><?PHP echo tiposEnlaces($row->Enlace );?></div></td>
			   ...
			   <td align="center"><a href="transparenciaWeb.php?id=<?=$row->id?>&Accion=<?=$AccionMod;?>"><img src="img/curso.gif" width="30" height="30" border="0" /></a></td>
...
<form id="formnoticias"   name="formnoticias" method="post" action="transparenciaAdmin.php" enctype="multipart/form-data">
...
<select name="enlace" id="enlace"><?php comboTipoEnlace("transparenciaTipo",$enlace); ?></select>
</select></fieldset>
