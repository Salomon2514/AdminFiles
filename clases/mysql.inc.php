...
class mod_db{
	...
	function insert($tb_name,$cols,$val,$astriction){
		...
		function Multiplesinsert_Asistencia($tb_name,$cols,$val){
			//HorariosMixtos_edicion.php
		if(empty($this->sql_link)){
			$this->connection();
		}


		$consultaInsercion = "INSERT INTO $tb_name ($cols)  VALUES $val";
		//echo "imprimiendo la consulta  ".$consultaInsercion."<br>";
		$insert = mysql_query("INSERT INTO $tb_name  ($cols)  VALUES $val",$this->sql_link);
			
			if($this->debug == 1){
				echo "hubo un eerror en la Inserci�n <br>";
				echo "Insert String = Insert into $tb_name $cols Values($val) $ast<br>";
				echo mysql_error();
			}

			if(!$insert){
				echo "el valor de val es: ".$val."<br>";
				 echo "<script>alert('Error en la Inserci�n de Datos een la cadena FIN DE LA CADENA...');</script>"; // English Error Message
				// echo $val;
	...
	function Multiplesinsert($tb_name,$cols,$val){
		if(empty($this->sql_link)){
			$this->connection();
		}
	...
	function del($tb_name,$astriction){
		...
		//echo  "DELETE FROM $tb_name $ast<br>";
	...
	function CantidadRegistros($string="",$qid=""){
		//$consulta= "SELECT id FROM contactos WHERE id='$idUsuarioL'";
			//$id2 = mysql_query($sql);
			...
			//El n�mero de filas de un conjunto de resultados en caso de �xito o FALSE en caso de error.
			//$resultado = mysql_query("SELECT * FROM tabla1", $enlace);
			$resultado = mysql_query($string, $this->sql_link);
			$reg = mysql_fetch_object($resultado);

			//echo "$n�mero_filas Filas\n";
